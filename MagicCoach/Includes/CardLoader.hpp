...
class CardLoader {
	...
	public:
		...
		Entity getCard(const unsigned int id, const unsigned int owner, const unsigned int controller);
		//Make a copy of loaded card, and return it

	private:

		Entity loadFromFile(const std::string path, const unsigned int id, const unsigned int owner, const unsigned int controller);
